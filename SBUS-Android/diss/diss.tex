...
Word Count:         & \bf 9627\footnotemark[1] \\
...
The first stage is to extract the top level of the desired message, in this case {\tt place/coordinates}. 
This message is then repackaged, using the lookup table to pack messages under the component's schema field names and inserting empty values for any missing fields. 
Based on the structure of the desired message, and the values in the lookup table, empty values can be inserted at the appropriate places in the message. 
Finally, if the component schema had additional layers rather than the peer schema, the message would be wrapped in these layers, with any other values which are missing being inserted. 
