...
Word Count:         & \bf 9486\footnotemark[1] \\
...
Here, {\tt place} is the only extra layer. 
...
%TODO: updating subscriptions
After the lookup table has been created, any predefined subscription filter can be sent to the peer. 
...
If the component schema has additional ancestors, then these are removed from the filter, while if the target schema has additional ancestors, the filter is prepended with these, to make the filter {\tt place/location/gps/latitude > 0.0}.
All other field names are then replaced with their corresponding entry in the lookup table, creating a filter which when applied to the target schema, has the expected effect, namely {\tt place/coordinates/position/latitude > 0.0}.
