...
Word Count:         & \bf 10056\footnotemark[1] \\
...
\Function{Repack}{$message$}
	\If{message has additional layers}
		\State \Call{UnpackLayers}{$message$}
	\EndIf
...
The Phone Manager (PM) is an Android application which is responsible for detecting changes in context and reconfigures other applications' connections. 

%TODO: detecting context - AIRS
The Phone Manager uses the AIRS application\footnote{\url{https://play.google.com/store/apps/details?id=com.airs}} to detect changes in context. 
An AIRS-SBUS gateway application has been created which acts as a remote server for sensor readings from AIRS, outputting them as events on an SBUS endpoint. 
This server was based on the AIRS code\footnote{\url{https://github.com/dirktrossen/AIRS}}, and then packaging any sensor readings as SBUS events and publishing them on the endpoint. 
The PM connects to this endpoint, thus can receive events for any sensor readings. 
However, AIRS only outputs sensor readings for a given sensor if a subscription has been enabled for that sensor. 
The PM must first subscribe to the appropriate sensors, via a ``control'' endpoint, in order to receive these readings. 
Conceptually this AIRS-SBUS gateway could also act as a composite event detector. 
It could collate multiple sensor readings and if these readings indicated a context change, output a single event. 
The PM provides the infrastructure to respond to any kind of event. 
Additionally, the PM can act as a receiver for system intents\footnote{\url{http://developer.android.com/reference/android/content/Intent.html
}}. 
In particular, the action {\tt NETWORK\_STATE\_CHANGED\_ACTION} can be used to detect a Wi-Fi change. 

%TODO: registering components + setting policies
In order to have their connections automatically reconfigured, applications need to register with the Phone Manager. 
They register in exactly the same way as they would with an RDC. 
In this sense, the PM is a lightweight RDC which does not offer search functionality. 
Once an application has registered with the PM, it can set {\sl map policies}. 
These policies determine how applications will have their connections reconfigured. 
A policy can either contain an IP address, or an RDC query. 
The PM will apply these policies to applications exactly as specified, which will either map them to an IP address, or cause them to perform an RDC query, and map to one of the results. 

%TODO: RDC updates 
In some context changes, the available RDCs may change. 
The Phone Manager currently informs applications of available RDCs on every change, via a builtin endpoint. 
Upon receiving an event on this endpoint, applications can either automatically accept the change, adding or removing the RDC, or perform a callback method, perhaps prompting the user to make a decision. 

%TODO: where to get RDC address from

%TODO: apply map policies
Once the PM has sent the RDC update to every registered application, it can apply applications' map policies. 
It simply send applications a message containing the parameters of the policy on another builtin endpoint, and applications then map according to this policy. 
