...
Word Count:         & \bf 8069\footnotemark[1] \\
...
When searching for a component using a particular data type, an application may be interested in an exact match, where names as well as types are matched, or may only be interested in the types. 
A component can only know about its own schema in advance, therefore queries must be specified in terms of the component's own schema.
This have been introduced as two new query parameters, where {\tt +H} represent an exact match, and {\tt +S} represent a type match. 
...
{\tt +Scity}		& Match the type of {\tt city} (a {\tt txt} field) \\
{\tt +Hlongitude}	& Match the {\tt longitude} field exactly \\

{\tt +Sgps}			& Match the {\tt gps} substructure type \\
{\tt +Hgps}			& Match the {\tt gps} substructure exactly \\

{\tt +Hcountry}		& {\tt <ignored>} \\
{\tt +Hlocation}	& Match the entire {\tt location} schema exactly \\

{\tt +Sgps+Hlongitude}	& Match {\tt gps} type AND match {\tt longitude} exactly \\
...
This would allow a query to be specified that would match a structure which has renamed {\tt altitude} to {\tt height}, but not one which has renamed {\tt longitude} to {\tt long}. 
The latter could be matched in the current case, if a the {\tt longitude} field existed elsewhere in that schema. 

This usage is implemented using a nested syntax, and Table \ref{tab:nested_schema_syntax} demonstrates some possible nested queries, based on the schema in Figure \ref{fig:locationschema}. 
...
{\tt +Sgps}							& Match a structure with the type of {\tt gps} \\
{\tt +Sgps[+Hlongitude]}			& Match a structure with the type of {\tt gps}, if it contains a field matching {\tt longitude} exactly \\
{\tt +Sgps[+Hlongitude+Hlatitude]}	& Match a structure with the type of {\tt gps}, if it contains fields matching {\tt longitude} and {\tt latitude} exactly \\

{\tt +Sgps[+Hlongitude]+Hcity}	& Match a structure with the type of {\tt gps}, if it contains a field matching {\tt longitude} exactly AND match the field {\tt city} exactly \\

{\tt +Sgps[+Slongitude]}	& Match a structure with the type of {\tt gps}, if it contains a field matching the type of {\tt longitude} \\
{\tt +Hgps[+Slongitude]}	& Match a structure which matches {\tt gps} exactly, if it contains a field matching the type of {\tt longitude} \\
...
A parser was already implemented for reading constraints from a string, so these new constraint types were added to that same parser. 
When one of this constraints was encountered, an item was added to the current list of schema constraints, containing the field name, whether this is an exact match or a type match, and a pointer to a list of child constraints. 
Upon reading an opening bracket, the current list being added to was added to a stack, and the previous item's list became the main list which constraints were added to. 
When reading a closing bracket, the previous list was popped off the stack, and new constraints are added to that.
%TODO: psuedocode?
