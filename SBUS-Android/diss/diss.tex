...
Word Count:         & \bf 8663\footnotemark[1] \\
...
The RDC returns all matching endpoints, not just the first match, so compares the query with each endpoint. 
...
\Function{CompareSchema}{$constraints$, $schema$}
	\ForAll{$constraint \in constraints$}
		\State $exact\gets constraint.exact$
		\If{$constraint.hash \equiv schema.hash(exact)$}
			\If{$exact$}
