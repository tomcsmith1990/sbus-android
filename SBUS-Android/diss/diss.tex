...
A messaging middleware was modified to allow searching for peers by aspects of their message schema, offering exact matches based on names and types, or looser matches which only consider types.
...
Upon detection of a change in context, this application can inform other applications of the change and automatically reconfigure their connections to connect them to new peers, based on policies the appplications define.
...
It then describes how the Phone Manager is responsible for detecting changes in context and reconfiguring applications' connections upon a change, and how integration with AIRS provides a way to monitor context. 
...
If internal memory was not an issue on the device, each application could store its own copy of {\tt sbuswrapper} in its filespace, and log to its own filespace. 
...
This is inline with the goals of pervasive computing, achieving scalability by allowing applications to use any relevant peer, and allowing applications to adapt to use heterogeneous content, all while being invisible to both the user and developer.
