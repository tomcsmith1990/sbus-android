...
Word Count:         & \bf 3694\footnotemark[1] \\
...
Figure \ref{fig:system_overview} shows the interactions that occur within the system, detailing how applications are reconfigured upon a change in context. 

\begin{enumerate}

\item After an application has registered with the Phone Manager, the application can send policies to the Phone Manager, which specify how the application endpoints should be reconfigured upon a change in context.
...
\item The Phone Manager processes these events to determine whether a change in context has occurred. Once a change happens, the Phone Manager informs all of the components registered of whether an RDC should be added or removed.

\item The application will either automatically accept this RDC change, registering or deregistering with the RDC, or perform some callback method, dependent upon its preferences.

\item The Phone Manager applies all of the policies within its policy table, instructing application endpoints to map to different components. These policies have been specified by the application, and may contain an IP address, or some constraint for a component to be mapped to.

\item If the policy specifies a constraint, the application will query the RDC for components matching this constraint. A constraint may be a component name, or schema constraints.

\item The RDC searches components registered with it for those matching the constraints, and returns the list of results to the application.

\item The application endpoint is then mapped to a new component in the environment, allowing communication to continue.
...
Healthcare & HeartRate & 192.168.0.10:44444 & HeartRate \\
Transport & Train & +NTransportProducer & Train \\
Transport & Transport & +Scoach & NULL \\
...
In the second row, the query is simply for a component named ``TransportProducer'', with an endpoint named ``Train''. 
In the third row, the query allows for any component which has a structure matching the type of {\tt coach}, where {\tt NULL} means the remote endpoint name doesn't matter. 

The Phone Manager currently assumes that changes in context are limited to changes in Wi-Fi network. 
However, by using sensor readings from AIRS, the policies could easily be extended to an event-condition-action model.
...
Accelerometer & \begin{math} val \ge50 \end{math} & Healthcare & 192.168.0.10:44444 \\
Time & \begin{math} 12 \le val.hour \le 14 \end{math} & Adverts & +IRestaurant \\
Wi-Fi & \begin{math} val = true \end{math} & Transport & +NTransportProducer \\
Wi-Fi & \begin{math} val = true \end{math} & Transport & +Scoach \\
...
As shown in Table \ref{tab:event_condition_action}, applications would not only specify their policy, which is the action they wish to take, but also the events and conditions they wish to take that action in. 
This would allow the healthcare system to specify that it only wishes for this mapping to occur if a high accelerometer value was measured, which might indicate that a fall has happened. 
An application receiving adverts might connect to adverts from nearby restaurants around lunchtime. 
...
% how this could do the constraints in issues section.
