...
Word Count:         & \bf 5944\footnotemark[1] \\
...
Figure \ref{fig:rdc_search_optimisations} shows the time taken for an RDC to search through all registered components to match schema constraints for different numbers of components. 
...
In a real environment, many RDCs are likely to have large numbers of components registered and as such these optimisations become more important, to ensure that any one schema search does not impact performance of other components too greatly.
Even in the worst case, the RDC can perform the search with 100 other components registered in just over 40 milliseconds, so should not present a bottleneck given expected usage.
...
\caption{Connection Times Between Producers and Consumers on a Laptop and Phone}
\label{tab:map_times}
\end{table}
...
\caption{JNI Timing Breakdown}
\label{fig:jni_times}
\end{figure}

Figure \ref{fig:jni_times} shows the breakdown of time taken to perform an SBUS function call via JNI, in this case emit(). 
This clearly shows that the use of JNI introduces a delay, almost five milliseconds here. 
Each call to an SBUS function using the JNI wrapper will incur some delay, meaning that applications on a phone will never operate as quickly as those written in C++.
However, this delay is known and unlikely to vary, and overall it should not cause any problems under normal circumstances.
