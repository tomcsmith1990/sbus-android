...
The Phone Manager informs applications of RDC changes on every network context change, publishing an event containing the RDC address, and whether it has became available or unavailable, according to whether the device has left or joined the network. 
The RDC change event is published to each registered application via a builtin endpoint. 
Upon receiving the event, applications can automatically accept the change, registering with or deregistering from the RDC address, and perform a callback method, perhaps prompting the user to make a decision. 
...
While this problem might be solved by implement a condition requiring the network signal to be greater than a given threshold, meaning reconfiguration would not occur until the user moves firmly into the network, an alternative solution might use a composite event detector. 
...
The system is in line with an event-condition-action model, using sensor readings (events) to map application endpoint (actions) whenever the sensor readings satisfy some conditions (conditions). 
...
However, queries allow applications to be more flexible in their peers. 
...
In the second row, an application may display adverts from nearby restaurants if it is around lunchtime. 
In the third row, the transport application wants to connect specifically to a Stagecoach component with an endpoint named ``Coach'' if a Wi-Fi connection is established. 
However, if a GPS event indicates that the user were in France, the application is more lenient and willing to connect to any component with an endpoint offering a structure matching the type of {\tt coach}.
...
Accelerometer 	& val $\ge50$ 				& 128.232.0.20:44444 	& HeartRate\\
Time			& val.hour = 12 			& +IRestaurant			& Advert \\
Wi-Fi 			& val = true 				& +NStagecoach 			& Coach \\
GPS 			& location(val) = France 	& +Scoach 				& NULL \\
...
The system could easily be extended to allow applications to specify policies for {\sl other} applications, which would then allow the user to set custom policies for any application. 
...
This would require some ``policy authoring'' application, which provides a simple interface to the user to set policies for any application on the device. 
This moves futher towards the pervasive vision, providing invisibility \cite{saha2003pervasive} in the system by requiring minimal user intervention, yet allowing the user to fine tune settings so that the system can meet more of the user's expectations. 
