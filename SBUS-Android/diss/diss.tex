...
Word Count:         & \bf 7441\footnotemark[1] \\
...
Android applications can register to receive intents\footnote{http://developer.android.com/reference/android/content/Intent.html} which indicate events such as a network change, or an incoming phone call. 
Furthermore, the AIRS application\footnote{https://play.google.com/store/apps/details?id=com.airs} observes sensor readings, and can output them to a remote server.
...
ARM-based devices, and Android platform 9 where chosen here, since these corresponded to the device which was available. 
...
The toolchain setup did include a bug\footnote{https://code.google.com/p/android/issues/detail?id=35279} which meant some include paths were missing, so {\tt <limits>} could not be referenced. A workaround for this is to fix the directory structure.

The Android version of {\tt pwd.h} which defines {\tt struct passwd} contains no member {\tt pw\_gecos}. 
This corresponds to the user's full name, which was used by SBUS to set the {\tt creator} field in a component's metadata. 
The creator was simply replaced by the string `Android'.

One of the greatest problems by far was that {\tt <sys/sockio.h>} was missing from Android.
SBUS uses this to determine the IP address which a component is running on, which is then used as part of connection setup. 
This meant that the entire method to determine the IP address had to be rewritten to work on Android. 
This was achieved using {\tt ioctl()}, to return a list of network interfaces and their IP addresses. 
From this list, the appropriate interface was chosen by name, and the IP address returned. 
The Wi-Fi address, if connected, was prioritised over the mobile network address.

Another large issue was the Android filesystem. 
SBUS invokes a wrapper, typically residing at {\tt /usr/bin/sbuswrapper} which means it can be invoked via a call to {\tt sbuswrapper}, since the {\tt PATH} environment variable contains {\tt /usr/bin}. 
This wrapper is responsible for handling the low level messages between components, which are then passed to the library level as structured messages. 
...
The Android SDK provides the Android Debug Bridge ({\tt adb}) to push and pull files to and from a device. 
On the Android emulator, {\tt /system} can be remounted as writeable\footnote{using {\tt mount -o rw,remount -t yaffs2 /dev/block/mtd3 /system }}, allowing {\tt sbuswrapper} to be pushed to {\tt /system/bin/sbuswrapper}. 
However, on a device without root access, {\tt /system/bin} cannot be remounted in this way, thus cannot be written to. 
This meant that a writeable directory was needed, and {\tt sbuswrapper} would have to be invoked using the full path. 
The SD card ({\tt /mnt/sdcard}) is writeable so was considered, however is mounted with the option {\tt noexec}, which means that although a file can be pushed to it, it cannot be executed. 
The best approach to use was to include {\tt sbuswrapper} as an asset to the application, and copy it to the application's filespace.
...
Since {\tt sbuswrapper} is large compared to the available internal memory on the device, it was infeasible to include a copy with every application. 
...
Although not an ideal solution, rooting the device would have devalued the project, since it would only then work on rooted devices.
...
When method is invoked on the Java object, the pointer is passed back to the native method, which then casts it to a pointer to the native object.
...
The project has been successful in this respect. Sensor readings from the AIRS application\footnote{https://play.google.com/store/apps/details?id=com.airs} and Android intents are used to detect a change in context. 
