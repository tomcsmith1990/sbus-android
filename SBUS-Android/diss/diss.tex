...
Word Count:         & \bf 4555\footnotemark[1] \\
...
This chapter first outlines the requirements of the system to be implemented. 
...
Finally, it describes how the system will be implemented.
...
% TODO: reword this probs.
John regularly travels from Cambridge to London. He uses his phone to check details of each part of his journey. From his home, he wants to know when the next taxi will be available to take him to the train station, and what time trains are leaving for London. Once he arrives in London, he wants details of Underground trains departing from whichever station he is in.

The following use cases show John's requirements of the system.

\begin{itemize}

\item {\bf Detecting and acting upon change in context}

\item {\bf Finding relevant components}

\item {\bf Communicating with different schemas}

\end{itemize}

\section{SBUS}

SBUS is a messaging middleware which can be used for stream based communication. 
...
Additionally, {\sl polymorphic} endpoints can be created which will accept any message type. 
...
This allows third party remapping to take place, where a component may remap another component's endpoints, or update its subscription filters.
...
\item How applications will find data sources relevant to them (constraints in terms of schema).

\item How applications will cope with unexpected data format - must be done at system level.

\item How change in context will be detected.

\item How applications' connections will be reconfigured (spoke map)

\item How the Phone Manager knows what to reconfigure to (policies)
