...
Word Count:         & \bf 10485\footnotemark[1] \\
...
The Phone Manager (PM) is an Android application which is responsible for detecting changes in context and reconfiguring other applications' connections. 
...
These intents are broadcast across the system on specific events, and by implementing a receiver, an action can be taken upon the event. 
In particular, the action {\tt NETWORK\_STATE\_CHANGED\_ACTION} can be used to detect a Wi-Fi change, allowing the Phone Manager to reconfigure connections based on a change in network. 

% AIRS
An AIRS-SBUS gateway application has been created which acts as a remote server for sensor readings from AIRS. 
AIRS sends sensor readings to this server, which outputs them as events on an SBUS endpoint. 
The server is based on the AIRS code\footnote{\url{https://github.com/dirktrossen/AIRS}}. 
After working out how readings are transmitted to a server, code was written to package sensor readings as SBUS events. 
AIRS can be ran in remote mode, to connect to the gateway, and each sensor reading is then packaged and published on an SBUS endpoint. 
The PM can then connect to this endpoint in order to receive an event every time a measurement is taken, harnessing any of the sensors on the device. 
When AIRS runs in remote mode, it only outputs sensor readings for a given sensor if a subscription has been enabled for that sensor. 
The Phone Manager must first subscribe to sensors it is interested in via a ``control'' endpoint. 
These subscriptions mean that the Phone Manager will only receive relevant events, and not be overrun with messages from all sensors.
