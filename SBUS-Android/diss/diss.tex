...
Project Title:      & \bf Automatic Configuration in Mobile Environments	\\
Examination:        & \bf Computer Science Tripos Part II, 2013 	\\
Word Count:         & \bf 10668\footnotemark[1] \\
...
Adaptation to different data formats masks heterogeneity in the data \cite{saha2003pervasive} from users, and indeed the application developers. 
...
This can be handled by modifying the data representation to match the expected format. 
...
This modified data can then be passed to the application, allowing the application to continue as if the data received has conformed to its own format. 
% subscription
...
The first shows how applications can find peers which offer the data they require, and how they can communicate with these peers despite differences in schemas. 
...
Flexible schema matching refers to components finding peers based on aspects of their schema, specifically whether or not their schema contains certain fields, and how components can communicate with these peers if their schemas are not identical. 
...
The project set out to create a system which would allow applications to automatically connect to different peers upon a change in context, where these peers may not necessarily present data in the format expected. 
The project has been successful in this respect. 
...
Through these policies, applications may control which peers they should be connected to, without requiring applications to find the peers themselves.
