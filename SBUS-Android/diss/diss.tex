...
Word Count:         & \bf 9063\footnotemark[1] \\
...
%TODO: other ways the search could be optimised - hash table to linked lists, search tree
...
This lookup table consists of a section containing names from the target schema, indexed by names from this schema, and a section containing names from this schema indexed by the target schema. 
These entries are added every time a successful match occurs in Algorithm \ref{alg:compare_schema}. 
Additionally, entries can be inserted for the children of a successful match, since the structure of the schemas must be the same. 
