...
This ensures that endpoints only ever receive messages matching that schema, from which values can be easily extracted since the structure of the message is known. 
...
When searching for components by data, an application may discover some component which provides the required data, but nested somewhere within a schema larger than its own. 
...
{\sl Is this needed? Not really got any changes. }
