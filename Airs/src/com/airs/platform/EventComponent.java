...
public class EventComponent implements Runnable 
{
	...
	public void run() 
	{
		...
		while (connected == true) 
		{
			...
			catch (Exception e) 
			{
				...
				System.out.println("CONNECTION FAILURE: Restart gateway!");
				return;
			}
		}
	}
	 ...
	 ***********************************************************************/
	synchronized public DIALOG_INFO Subscribe(byte FROM[], byte event_name[], byte event_body[], int body_length, int Expires, Callback callback)
	{
		...
		method.TO.string 	= current_TCPClient.IMEI.getBytes();
		method.TO.length 	= (short)current_TCPClient.IMEI.length();
		method.FROM.string	= FROM;
		method.FROM.length	= (short)FROM.length;
	...
	boolean Dispatch(Method current_method)
	{
		...
		case method_type.method_PUBLISH:
			...
			if ((current_dialog = find_Dialog(current_method.pub.e_tag, true)) != null) {
				...
				if ((current_callback = find_EventUA(current_method.event_name.string)) != null) {
					if ((current_dialog = create_Dialog(current_method, current_callback, dialog_state.PUBLICATION_PENDING, true)) != null) {
						current_callback.callback(current_dialog);
					} else {
						debug("...could not create dialog for this method!");
					}
				} else {
		...
		case method_type.method_CONFIRM:
			...
			if ((current_dialog = find_Dialog(current_method.conf.dialog.dialog_id, false)) != null)
						...
						||(current_dialog.dialog_state == dialog_state.SUBSCRIPTION_TERMINATED_CLIENT))
				{
					...
					if (current_dialog.dialog_state == dialog_state.SUBSCRIPTION_TERMINATED_CLIENT 
							|| current_dialog.dialog_state == dialog_state.SUBSCRIPTION_TERMINATED_SERVER) {
						debug("EventComponent::Dispatch:delete dialog and callback");
						delete_Dialog(current_dialog, false);
					}
				} else {
			...
			{
				// is there an existing incoming dialog?
				if ((current_dialog = find_Dialog(current_method.conf.dialog.dialog_id, true)) != null)
				{
					// has anything been sent before (SUBSCRIBE, NOTIFY, BYE, or PUBLISH)?
	...
	DIALOG_INFO	find_Dialog(int dialog_id, boolean direction)
	{
		DIALOG_INFO	search;
